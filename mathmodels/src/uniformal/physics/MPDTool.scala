...
class MPDTool(controller: Controller) {
  ...
  val unicodeLatexMapRegex = """(.+)\|(.+)""".r  
  
  val unicodeToLatexList = {
    Source.fromResource("unicode-latex-map").getLines().map(
      x => unicodeLatexMapRegex.findAllIn(x).matchData.map{
        m: Regex.Match => (m.group(2), m.group(1))
      }.toList.head
    ).toList
  }
  
  val supportedUnicodeList = unicodeToLatexList.map(_._1)
                  ...
                  case construct_underscore_geometry(f) =>
                    f match {
                      case geometry_underscore_description(s) =>
                        Some(GeometryDef(c.parent, c.name, GConstructionFromDescription(s.toString)))
                      ...
                      case FunTerm(pargs, body) => 
                        ...
                        Some(GeometryDef(c.parent, c.name, 
                                          GConstructionFromPredicate(Booleans.MakeBooleanStructureFromTerm(body, pargs.map(x => (Some(x._1), x._2))), pargs)
                                         ))
                      case _ =>
  ...
  def toMPD(thy: DeclaredTheory): Option[MPD] = {
    ...
    comps foreach { 
      comp => comp match {
        case qd if qd.isInstanceOf[QuantityDecl]  => {
          var newQd = qd.asInstanceOf[QuantityDecl]
          quantityDecls.indexWhere(_.name.toString == newQd.name.toString) match {
            case -1 => quantityDecls ::= qd.asInstanceOf[QuantityDecl]
            case i => quantityDecls(i).df = newQd.df
          }
        ...
        case geom if geom.isInstanceOf[GeometryDef] => {
          val newGeomDef = geom.asInstanceOf[GeometryDef]
          println(geometryDecls, newGeomDef)
          val geomIndex = geometryDecls.indexWhere(_.name == newGeomDef.name)
          if (geomIndex == -1)
            geometryDecls ::= GeometryDecl(newGeomDef.parent, 
                                           newGeomDef.name,
                                           Some(newGeomDef.domain))
          else
            geometryDecls = geometryDecls.updated(geomIndex, GeometryDecl(newGeomDef.parent, 
                                                                          newGeomDef.name,
                                                                          Some(newGeomDef.domain)))
